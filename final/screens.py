...
import math
import sys
from config import WIDTH, HEIGHT, BLACK, WHITE, BLUE, RED, GREEN, YELLOW, MAX_SKILL_LEVEL
from core import Button, draw_panel
from entities import Player, Enemy, MagicGun, FireBall, ElectricShock, ProtectShield

# -----------------------------
# 1. Game Controller (Logic Loop Runner)
# -----------------------------
class GameController:
    ...
    def tick_logic(self, dt, gs):
        ...
        for w in gs.weapons: w.update(dt, gs)
        ...
        # 충돌 처리
        self._handle_collisions(gs, dt)

        # ✅ [최적화] 모든 엔티티 리스트를 한 번에 필터링 (렉 방지 및 유령 잔상 제거)
        self.enemies = [e for e in self.enemies if e.alive()]
        self.bullets = [b for b in self.bullets if not b.out_of_bounds()]
        self.skill_projectiles = [sp for sp in self.skill_projectiles if not sp.dead()]

    def _spawn_routine(self, gs):
        if not self.midboss_spawned and self.elapsed >= 120: self._spawn_boss(gs, "midboss")
        if not self.finalboss_spawned and self.elapsed >= 240: self._spawn_boss(gs, "finalboss")
        if len(self.enemies) < 60:
            ...
            self.enemies.append(Enemy(kind, (x, y), 10, 10, gs.img_spider if kind=="spider" else gs.img_skull))

    def _spawn_boss(self, gs, kind):
        is_mid = (kind == "midboss")
        e = Enemy(kind, (WIDTH*0.7, HEIGHT*0.4), 500 if is_mid else 1000, 500 if is_mid else 0, gs.img_midboss if is_mid else gs.img_finalboss, 36 if is_mid else 72)
        e.speed = 480 if is_mid else 260; self.enemies.append(e); self.active_boss_kind = kind; self.boss_deadline = self.elapsed + 60.0
        if is_mid: self.midboss_spawned = True
        else: self.finalboss_spawned = True

    def _handle_collisions(self, gs, dt):
        # ✅ [최적화] .length() 대신 .length_squared()를 사용하여 제곱근 연산 제거
        ...
        for e in self.enemies:
            ...
            if dist_sq <= min_dist**2:
                ...
                if dist_sq > 0:
                    ...
                    e.pos = p_pos + push_dir * (min_dist + 1)
                else:
            ...
            for b in self.bullets:
                if (e.pos - b.pos).length_squared() <= (e.radius + b.radius)**2:
                    e.hp -= b.damage
                    # 리스트에서 직접 지우지 않고 범위 밖으로 보내서 다음 필터링에서 제거되게 함
                    b.pos.x = -1000 
            
            # 스킬 투사체 충돌
            for sp in self.skill_projectiles:
                if (e.pos - sp.pos).length_squared() <= (e.radius + sp.radius)**2:
                    e.hp -= sp.damage
                    sp.hit_count += 1
            ...
            if not e.alive():
                if e.kind in ("spider", "skull"):
                    self.player.kills += 1
                    if self.player.add_exp(e.exp_reward): gs.trigger_level_up()
                elif e.kind == "midboss":
                    self.player.add_exp(500); self.boss_deadline = None; self.active_boss_kind = None
...
class PauseOverlay:
    def __init__(self, gs):
        ...
        self.btn_font = pygame.font.SysFont("malgungothic", 28)
        self.btn_resume = Button((WIDTH // 2 - 200, HEIGHT // 2 + 40, 400, 70), "계속하기 (P)", BLUE, (95, 185, 255), self.btn_font)
...
class SkillChoiceOverlay:
    def __init__(self, gs):
        ...
        self.font = pygame.font.SysFont("malgungothic", 22)
        self.name_font = pygame.font.SysFont("malgungothic", 28) # ✅ [최적화] 폰트 사전 로드
    ...
    def handle_event(self, event):
        ...
        if 0 <= idx < len(self.options):
            self.options[idx].acquire_or_level(); self.active = False; self.gs.bgm.unpause()
    ...
    def draw(self, surf):
        ...
        for i, r in enumerate(self.btn_rects):
            ...
            # ✅ [최적화] 매 프레임 SysFont 호출하던 버그 수정
            surf.blit(self.name_font.render(wpn.name, True, WHITE), (r.x + 22, r.y + 24))
...
class GameScreen:
    ...
    def finish_game(self, success, reason):
        self.bgm.stop()
        stats = {"survival_time": self.controller.elapsed, "kill_count": self.controller.player.kills, "player_config": self.controller.player_config, "reason": reason}
    ...
    def update(self, dt):
        if not self.paused and self.overlay is None:
            ...
            if self.controller.elapsed >= self.controller.total_time: self.finish_game(False, "시간 종료")
    ...
    def _draw_hud(self, surf):
        c = self.controller; bx, by, bw, bh = 30, 18, 520, 22
        ...
        rem = max(0.0, c.total_time - c.elapsed); surf.blit(self.info_font.render(f"{int(rem//60)}:{int(rem%60):02d}", True, WHITE), (WIDTH - 420, 14))
