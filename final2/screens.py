...
class StartScreen:
    def __init__(self, mgr, rm, audio):
        ...
        self.image_paths = ["player1.png", "player2.png", "player3.png"]
    ...
    def draw(self, surf):
        ...
        for i, r in enumerate(self.card_rects):
            ...
            img_a = pygame.Rect(r.x+20, r.y+20, r.w-40, 190)
            pygame.draw.rect(surf, (60, 60, 70), img_a, border_radius=12)
            ...
            for j, k in enumerate(["HP", "VEL", "DMG"]):
                val = self.PLAYERS[i][k]
                val_str = f"x{val}" if k == "DMG" else str(val)
        ...
        help_lines = ["[플레이어]", "P1: HP 높음", "P2: 이동속도 높음", "P3: 공격력 높음", "", "[인게임]", "레벨업: EXP 획득 시", "레벨업 시 스킬 선택", "", "[조작]", "이동: WASD / 방향키"]
        for ln in help_lines:
            surf.blit(self.font.render(ln, True, (215, 215, 225)), (self.help_rect.x + 18, yy))
            yy += 28
...
class GameScreen:
    ...
    def draw(self, surf):
        surf.fill(BLACK); arena = pygame.Rect(20, 70, WIDTH-40, HEIGHT-90)
    ...
    def _draw_hud(self, surf):
        ...
        pygame.draw.rect(surf, (45,45,55), (bx, by, bw, bh), border_radius=10)
        pygame.draw.rect(surf, BLUE, (bx, by, int(bw * (c.player.exp/c.player.exp_need)), bh), border_radius=10)
        ...
        surf.blit(self.info_font.render(f"{int(rem//60)}:{int(rem%60):02d}", True, WHITE), (WIDTH - 420, 14))
...
class EndScreen:
    ...
    def handle_event(self, event):
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_RETURN: self.mgr.set(GameScreen(self.mgr, self.stats["player_config"], self.rm, self.audio))
            if event.key == pygame.K_ESCAPE: self.mgr.set(StartScreen(self.mgr, self.rm, self.audio))
        if self.btn_restart.clicked(event): self.mgr.set(GameScreen(self.mgr, self.stats["player_config"], self.rm, self.audio))
        if self.btn_to_start.clicked(event): self.mgr.set(StartScreen(self.mgr, self.rm, self.audio))
